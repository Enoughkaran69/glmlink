...
export default async function SearchResultsPage({ params }) {
  ...
  const productResults = allProducts.filter(p => p.id === `p${query}` || String(p.price) === query || p.name?.includes(query));
  const reelResults = allReels.filter(r => r.id === `r${query}` || r.title?.includes(query));
      ...
      <div className={styles.mainContent}>
        ...
        <>
            ...
            <div className={styles.reelsGrid}>
              {reelResults.length > 0 ? reelResults.map(reel => (
                <div key={reel.id} className={styles.reelCard}>
                  ...
                  <div className={styles.reelTitle}>
                    <a href={`/reel/${reel.id}`} className={styles.navLink} style={{ color: '#9333ea', textDecoration: 'underline', cursor: 'pointer' }}>
                      <h4>{`${reel.id.replace('r', '')}. ${reel.title || "Untitled Reel"}`}</h4>
                    </a>
                  </div>
                </div>
